---
title: "Helpers"
permalink: /getting_started/helpers/
...
Designed to compliment the [`splash`]({{ "/getting_started/layouts/#splash-page-layout" | relative_url }}) page layout as a way of arranging and aligning "feature blocks" containing text or image.
...
| **btn_class**  | Optional     | Button style. See [utility classes]({{ "/getting_started/utility-classes/#buttons" | relative_url }}) for options. | `btn` |
...
**1.** Start by adding a set of titles and URLs to `_data/navigation.yml` in the same way the [`main` navigation]({{ "/getting_started/navigation/" | relative_url }}) is built.
...
The theme's documentation is built with the `nav_list` helper so if you'd like an example to dissect take a look at `navigation.yml`, `_config.yml` and `_doc` collection in the [`/getting_started/` folder](https://github.com/{{ site.repository }}/tree/master/getting_started/) of this repo.
