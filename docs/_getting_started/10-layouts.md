---
title: "Layouts"
permalink: /getting_started/layouts/
...
Assign with `layout: single` , or better yet apply as a [Front Matter default]({{ "/getting_started/configuration/#front-matter-defaults" | relative_url }}) in `_config.yml`.
...
**Note:** More information on using this `_include` can be found under [**Helpers**]({{ "/getting_started/helpers/" | relative_url }}).
{: .notice--info}

## Taxonomy Archives

If you have the luxury of using Jekyll plugins, the creation of category and tag archives is greatly simplified. Simply enable support for the [`jekyll-archives`](https://github.com/jekyll/jekyll-archives) plugin with a few `_config.yml` settings as noted in the [**Configuration**]({{ "/getting_started/configuration/#archive-settings" | relative_url }}) section and you're good to go.
...
A derivative archive page layout to be used as a simple home page. It is built to show a paginated list of recent posts based off of the [pagination settings]({{ "/getting_started/configuration/#paginate" | relative_url }}) in `_config.yml`.
...
**Note:** Jekyll can only paginate a single `index.html` file. If you'd like to paginate more pages (e.g. category indexes) you'll need the help of a custom plugin. For more pagination related settings check the [**Configuration**]({{ "/getting_started/configuration/#paginate" | relative_url }}) section.
{: .notice--info}
